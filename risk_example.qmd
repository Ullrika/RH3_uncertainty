...
Sources of uncertainty can be summarised as tables with explanations and (but this is optional) indications on their influence on the overall uncertainty (using qualitative expressions such as --- or ++).
...
This judgement is relatively easy when the conservative estimate for the Highest Exposure (here `r heu`) is far below (a conservative value for) the Point of Departure (here `r bmdl`).

Conservative values can be obtained by uncertainty factors. It is preferable to have well-calibrated uncertainty factors to emsure that they result in estimates with known level of conservatism. If not, it is difficult for the experts to judge their uncertainty in the conclusion.

In our hypothetical example, the expert agree that their uncertainty lies in the inconclusive range. Some of them even said that they could not make this judgement, which would also lead to an inconclusive assessment.
...
::: callout-note
A humble note from Ullrika and Dawei: The numbers and probabilities are chosen for demonstration. There are inconsistencies in the judgements between probability and numbers first in this example. In reality, only one of the options are chosen. Due to this inconsistency, the next tier will not be consistent with previous ones.
:::
...
A probability distribution representing uncertainty in the **Point of Departure** can be taken from the BMD uncertainty distribution for the most sensititive endpoint generated by bootstrapping or Bayesian modelling. There are plenty of BMD software available to do this.

::: callout-note
Note that the estimate is coming from the most sensitive endpoint for which there is enough data to perform a reliable BMD modelling. The utlimate goal of uncertainty analysis is to consider the combined impact of all sources of uncertainty, it includes taking into account thoese endpoints where there is no BMD modelling. This can be done in the last step in the assessment of overall uncertainty.
:::
...
The lower bound for the BMD is zero. Since the distribution is skewed to the right, the experts set an upper plausible bound. Here we fit a generalised beta distribution to these quantiles using the SHELF R-package.

```{r}
#| echo: true 

# this distribution was fitted earlier in the code
...
::: callout-warning
This approach ignores additional sources of uncertainty than those considered in the model, hence we need to consider those as well. e.g. by adjusting the specification of the quantiles.
:::
...
The final step is always a judgement made by experts of the influence of any additional sources of uncertainty. 
