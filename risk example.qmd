...
Also know as the glorious *Dawei formulas*
...
In this example, this ratio is $\frac{`r x`}{`r y`} = `r round(x/y,2)`$
...
$$P(PoD > y) = `r p_PoD`\%$$ That implies that the conservative estimate for the hazard is $y = `r bmdl`$
...
Practical certainty is obtained if $$100 - [(100-P(HE < x)) + (100-P(PoD > y))] \geq `r 100*pc_yes`$$ In this example, the left hand side is $100-[`r 100 - p_PoD`+`r 100-p_HE`]=100-`r 100-p_PoD` + `r 100-p_HE`= `r 100-(100-p_PoD + 100-p_HE)`$
...
We (the assessors) let the BMDL, the BMD, and the BMDU define the 5th percentile (P05), the median (P50), and the 95th percentile (P95) of the probability distribution for uncertainty in the **PoD**.
