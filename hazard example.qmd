...
> A **probability bound** is a quantile for a continuous quantity associated with a probability (or bounded probability). For example, $q_{05}$ is the 5th quantile for the number $X$ if $P(X<q_{05}) = 5%$.
An example of a bounded probability bound (horrible terminology) can be $q_{\leq 05}$ is a bounded quantile for the number $X$ if $P(X<q_{\leq 05}) \leq 5%$. 
...
For this hazard identification the question is if exposure  at a given dose to compound $X$ can cause an effect in a chosen endpoint? This is a categorical questions with two possible answers NO and YES.
...
df_point=data.frame(y=rep(dist_to_50[ind_pcno],2),x=c(prior_yes_imp[ind_pcno],post_yes_imp_step2[ind_pcno]),type=c("prior","posterior"))
...
We also conclude we are betwee `r round(E123[2]*100)` and `r round((E123[2] + E123[3])*100)`% certain that it does not cause the effect.
