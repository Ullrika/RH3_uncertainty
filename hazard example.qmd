---
title: "Tiered approaches to characterise uncertainty in NGRA examplified with an hazard identification"
...
Thus, given the evidence (and what we know about the accuracy of the QSAR prediction) we are `r round(post_yes*100,0)`% certain that $X$ is an hazard.
...
$$ P(YES|-,-,+)=$$
$$=\frac{P(-,-,+|YES)P(YES)}{P(-,-,+|YES)P(YES)+P(-,-,+|NO)P(NO)}$$
...
The evidence is combing from a QSAR with a prediction stating that compound $X$ can cause an effect on the endpoint with a probability between `r round(E1[1]*100)` and `r round((E1[1] + E1[3])*100)` and it does not cause an effect with probability between `r round(E1[2]*100)` and `r round((E1[2] + E1[3])*100)`%.
