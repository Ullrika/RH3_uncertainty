...
subtitle: "RiskHunt3R meeting Aug 2024"
...
# numbers first
y_nfirst = bmdl
p_PoD = 100-pr_bmdl*100 # probability for y 
...
```{r}
# elicit probability
p_HE <- 100*plnorm(x_nfirst,fit_he$Normal$mean,fit_he$Normal$sd)
```
...
$\frac{HE}{PoD} = \frac{`r heu`}{`r bmdl`} = `r heu/bmdl`$ which is above the threshold.
...
In our example, the expert agree that their (un)certainty lies in the inconclusive range. Some of them say that they can not make this judgement, which would also leed to an inconclusive assessment.
...
Practical certainty is not reached with the probability bounds analaysis using probabilities first, proceed with a refined approach.
...
Practical certainty is obtained if $$100 - P(HE < x) - P(PoD > y) \geq `r 100*pc_no`$$ In this example, the left hand side is $100-[`r p_HE`+`r 100-p_PoD`]=100-`r p_HE` - `r 100-p_PoD`= `r 100-(p_HE + 100-p_PoD)`$

Hence, practical certainty is reached with the probability bounds analysis using numbers first.
...
::: callout-note
A humble note from Ullrika and Dawei: The numbers and probabilities are chosen for demonstration. There might be inconsistencies in the judgements between the "probability-first" and "numbers-first" approaches in this example. In reality, only one of the options are chosen.
:::
...
A probability distribution representing uncertainty in the estimates of High Exposure is informed by a Monte Carlo simulation sample of the 95th percentile of the Cmax in the target population. Here we have the quartiles from such a sample.

| min HE       | median HE    | max HE       |
|--------------|--------------|--------------|
| P5           | P50          | P95          |
| `r hel`      | `r hem`      | `r heu`      |
| `r log(hel)` | `r log(hem)` | `r log(heu)` |
...
$$ \sigma = \frac{P95 - \mu}{`r round(qnorm(0.95),2)`} =  \frac{`r log(heu)`-`r log(hem)`}{`r round(qnorm(0.95),2)`}=`r round((log(heu) - log(hem))/qnorm(0.95),2)`$$
...
Experts might judge that any additional sources of uncertainty will have little influence and that there is no need to adjust the calculated % probability that the risk ratio is less than 1.

If not, experts can choose to consider any additional uncertainties by adjusting the PoD or HE by an uncertainty factor f. This factor can be described as a probability distribution or have a single number. Here the experts argue that the exposure is overly conservative, and can be adjusted down by a factor, i.e. $f = 1/2$.
...
The distributions for High Exposure and the Point of Departure when considering the impact of additional sources of uncertainty overlap.

```{r}
pp11 <- ggplot(data.frame(dose = c(pod,he*f), quantity = rep(c("Point of Departure", "High Exposure/f"),each = niter)),aes(x=dose, fill = quantity)) +
  geom_density(alpha=0.2) +
xlim(0,20) +
scale_x_continuous(trans='log') +
  ggtitle("Uncertainty in High Exposure*f \n and the Point of Departure")
...
Here the expert judge that their % certainty for no health concern is `r round(mean(pod/(he*f)>1)*100)`%. 

```{r}
df <- density(he*f/pod, from=0)
df <- data.frame(x=df$x,y=df$y)
pp <- ggplot(data.frame(x=df$x,y=df$y),aes(x=x, y=y)) +
  geom_line() +
  geom_ribbon(data=subset(df,x<1),  aes(x=x,ymax=y),ymin=0,fill="red", alpha=0.5) +
...
#scale_x_continuous(trans='log') +
xlim(0,4) +
  ggtitle("Uncertainty in HE*f/PoD", subtitle=paste0("P(HE*f/PoD<1)=",round(mean(he*f/pod<1)*100),"%"))
pp
#ggsave("img/uncertaintyriskratio.png",height = 3)
```
