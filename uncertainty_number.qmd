...
subtitle: "RiskHunt3r"
author: "Ullrika Sahlin, Dawei Tang and Zheng Zhou"
format: 
  html:
    toc: true
    echo: false
...
- direct expert judgement on the outcome of interest
...
The quantity of interest for the hazard assessment is a Reference Point $RP$. 

As an example, a $RP$ of `r bmdl` μg `r cm`/kg bw per day was identified by the experts as the BMDL from a model average of BMD modelling for skin cancer. 
...
The range of the exposure estimates for `r cm` was assessed as 
...
The risk managers has informed the risk assessors that they find a certainty of `r p_DM`% or more as acceptable for decision making. This is know as practical certainty and it can be sufficient to conclude if overall uncertainty is above or below this limit. 
...
This is a useful option when the Highest Exposure $HE$ is far below the Reference Point $RP$.
...
In this example, the experts are asked to express their uncertainty in the Reference Point and in a High Exposure.

1) The experts judge that they are `r p_RP`% certain that the Reference Point is above `r bmdl` μg `r cm`/kg bw per day.  

$$P(RP > `r bmdl`) = `r p_RP`\%$$

| P below      | value   | P above |
|--------------|---------|---------|
| `r 100-p_RP`%| `r bmdl`|`r p_RP`%|
...
2) The experts then judge that they are `r p_HE`% certain that a High Exposure is below `r heu` μg `r cm`/kg bw per day.

$$P( HE < `r heu`) = `r p_HE`\%$$

| P below  | value  | P above     |
|----------|--------|-------------|
| `r p_HE`%| `r heu`|`r 100-p_HE`%|

3) Then we use probability bounds analysis (and the rule of "lost probabilities") to calculate that it is at least 
...
4) See if a conclusion can be made. If the value `r round(heu/bmdl,2)` is less than `r th` and the probability is above the practical certainty `r p_DM`, then result is enough to reach a conclusion whether a High Exposure is below the Reference Point (i.e. that the MOE is greater than `r th`). 

## Probabilities first

The refined option with probability bounds can be modified to consider the probability threshold for a "firm conclusion" (also known as the level for practical certainty). In this example, the risk managers and the experts have agreed that experts must be at least `r p_DM`% certain that a High Exposure $HE$ is below the Reference Point $RP$ in order for risk managers to make a decision. 
...
2) Divide this into two parts, one for hazard and one for exposure, here $`r p_lost_RP`\%$ and $`r p_lost_HE`\%$.

3) Subtract the lost probability for hazard from 100, to set the probability required for a conservative estimate of the Reference Point.

$$100\% - `r p_lost_RP`\% = `r 100-p_lost_RP`\%$$

4) Elicit a conservative estimate of the Reference Point (or RP/UF) from the toxicology experts (asking them to consider the identified sources of uncertainty for the hazard assessment). Define the conservative estimate as the quantile $y$

$$P(RP < y) = `r 100-p_lost_RP`\%$$

5) Subtract the lost probability for exposure from 100, to set the probability required for a conservative estimate of a High Exposure.

$$100\% - `r p_lost_HE`\% = `r 100-p_lost_HE`\%$$

6) Elicit a conservative estimate of a High Exposure from the exposure experts (asking them to consider the identified sources of uncertainty for the exposure assessment). Define the conservative estimate as the quantile $x$

$$P(HE < x) = `r 100-p_lost_HE`\%$$

7) Divide the conservative estimate for the exposure with the conservative estimate for the hazard $\frac{x}{y}$

9. If the ratio is less than the threshold on MOE, then there is sufficient certainty for a firm conclusion that exposure is below the hazardous concentration. I.e. is $\frac{x}{y} < `r th`$?
...
## Probability followed by number

An alternative approach is to use the RP as starting point and derive the probability from that. Then set the lost probability for exposure and elicit the quantile for exposure. 

1) Ask experts to judge their probability for a conservative estimate of hazard, e.g. the BMDL, $y = `r bmdl`$
...
2) Find a value on exposure on the critical threshold for the MOE

$$\frac{x}{y} = `r th`$$

$$x = `r th` \cdot y = `r th*bmdl`$$

3) Ask expert to judge their probability that the exposure is less than $x$. 

$$P(HE < x) = `r p_x`\%$$

4) Check if practical certainty has been obtained, i.e. if $100 - [(100-P(HE < x)) + (100-P(RP > y))] \geq `r p_DM`\%$  

$$100-[`r 100-p_x`+`r 100-p_y`]=100-`r 100-p_x + 100-p_y`=`r 100-(100-p_x + 100-p_y)`$$
::::
...
Here we fit a lognormal distribution using the SHELF R-package. 
...
Check if the level of certainty for the outcome of interest is `r round(mean(rp/he>1)*100)`% is in the region of practical certainty (i.e. <`r 100-p_DM`% or >`r p_DM`%). 

|Health concern|Inconclusive           |No health concern|
|--------------|-----------------------|-----------------|
|<`r 100-p_DM`%|`r 100-p_DM` - `r p_DM`|>`r p_DM`%       |

If not, the assessment should be reported as **inconclusive**. The risk managers can decide to support generation of new studies to fill data gaps and perform a re-evaluation of the risk assessment. 
